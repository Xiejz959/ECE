...
void arrSelectSort(int *values, int elems)
{
	int startScan, maxIndex, maxElem;
	for (startScan = 0; startScan < (elems - 1); startScan++)
	{
		maxIndex = startScan;
		maxElem = values[startScan];
		for(int index = startScan + 1; index < elems; index++)
		{
			if (values[index] > maxElem)
			{
				maxElem = values[index];
				maxIndex = index;
			}
		}
		values[maxIndex] = values[startScan];
		values[startScan] = maxElem;
	}
}
