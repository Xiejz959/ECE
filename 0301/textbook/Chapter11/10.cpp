...
void searchByTopic(const Speaker arr[], int n) {
    string key = readNonEmptyLine("Enter topic keyword to search: ");
    string keyLower = toLowerCopy(key);
    bool found = false;
    cout << "-- Search Results --" << endl;
    for (int i = 0; i < n; ++i) {
        if (!arr[i].occupied) continue;
        string topicLower = toLowerCopy(arr[i].topic);
    ...
    if (!found) {
        cout << "No speaker matches the given keyword." << endl;
    }
}
...
void printMenu() {
    ...
    cout << "5) Search speakers by topic keyword" << endl;
    cout << "6) Exit" << endl;
}
